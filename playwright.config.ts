 ...
 * and the PocketBase URL are available without a dependency. In CI these come
 * from the workflow env (the compose'd PocketBase runs on localhost).
 */
...
export default defineConfig({
  testDir,
  fullyParallel: true,
  // Cap concurrency: every worker hits the SAME local PocketBase, whose SQLite
  // writer serializes. A small pool trades a little wall-clock for stability
  // against shared state.
  workers: 4,
  forbidOnly: !!process.env.CI,
  retries: process.env.CI ? 2 : 0,
  reporter: process.env.CI ? [['list'], ['html', { open: 'never' }]] : 'list',
  // A multi-step authoring scenario (sign in → create page → add block →
  // reopen) does several reads/writes; 60s gives the whole scenario room.
  timeout: 60_000,
