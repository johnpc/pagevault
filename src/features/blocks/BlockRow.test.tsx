...
describe('BlockRow', () => {
  ...
  it('Delete at the end of a block merges the next one forward with the live value', async () => {
    const onMergeForward = vi.fn().mockReturnValue(true);
    render(
      <BlockRow
        block={mk({ id: 'b3', content: 'Hello' })}
        onEdit={vi.fn()}
        onRemove={vi.fn()}
        onSplit={vi.fn()}
        onMerge={vi.fn()}
    ...
    expect(onMergeForward).toHaveBeenCalledWith('b3', 'Hello');
  });

  it('Delete mid-text does not forward-merge (lets the textarea delete a char)', async () => {
    ...
    await userEvent.keyboard('{Delete}');
    expect(onMergeForward).not.toHaveBeenCalled();
  });

  it('selects the block when the ⋮⋮ handle is clicked', async () => {
    ...
    await userEvent.click(screen.getByLabelText(/click to select the block/i));
  ...
  it('inserts a block below via the "+" gutter button', async () => {
    const onInsertAfter = vi.fn();
    const block = mk({ content: 'here' });
    render(
      <BlockRow
        block={block}
        onEdit={vi.fn()}
    ...
    await userEvent.click(screen.getByLabelText('Add a block below'));
    expect(onInsertAfter).toHaveBeenCalledWith(block);
  });
