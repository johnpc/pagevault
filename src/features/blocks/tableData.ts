...
/** Make a typed column. Shared by emptyTable + the normalizer. */
export const col = (name: string, type: TableColumnType = 'text'): TableColumn => ({ name, type });
...
// normalize() (grid coercion + reference-preserving re-normalize) lives in
// tableNormalize.ts; re-exported so callers keep importing it from here.
export { normalize } from './tableNormalize';
